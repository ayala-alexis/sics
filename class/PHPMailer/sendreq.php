...
if(is_array($_REQUEST)) {
	...
	while ($row_user = mysqli_fetch_array($run_user)){
		if(!empty($row_user[2])  && $row_user[3] <> '1') {
			...
			$body = $row['prehreq_usuario'].' le ha enviado la solicitud #'.$row['prehreq_numero']. " de suministros para que usted la autorice.";
			$body .= "<br> desde (". $row_cc["cc_codigo"] .")".$row_cc["cc_descripcion"]." de ".$row_cc["emp_nombre"];
			if($cotis > 0){
				$body .= '<br>Favor revisar las cotizaciones adjuntas';
			}
			$mail->Subject = 'Requisicion para autorizacion #'.$row['prehreq_numero'];
			$mail->Body    = $body;
	...
	header('Location: http://192.168.40.6/sics/?c=req&a=inicio&id=6');
	die();
