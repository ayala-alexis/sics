...
function infoPreHSolc($idprehsol, $idcc, $idemp){
	try {
		$db = DB::getInstance ();
		$conf = Configuracion::getInstance ();
		$sql = "Select a.*,b.*,c.*,ct.id id_cat,ct.categoria desc_cat,u.usr_nombre usuario_cat From "
			.$conf->getTbl_prehsol()." a"
			." Join ".$conf->getTbl_empresa()." b On"
			." b.id_empresa = a.id_empresa"
			." Join ".$conf->getTbl_cecosto()." c On"
			." c.id_cc = a.id_cc"
			." left join cat_gerencias ct "
			." on ct.id = a.id_categoria "
			." left join usuario u "
			." on u.id_usuario in (ct.user_aproba_1,ct.user_aproba_2,ct.user_aproba_3) "
...
function detPreDSolc($idprehsol, $idcc, $idemp){
	...
	try {
		$db = DB::getInstance ();
		$conf = Configuracion::getInstance ();
		$sql = "Select d.*,CONCAT(p.descripcion_producto,' (',p.marca,')') item,c.descripcion_categoria cat From "
				.$conf->getTbl_predsol()." d "
				." left Join Fproductos p On "
				." p.id_producto = d.id_producto "
				." left join Fcategorias c on "
				." c.id_categoria = p.id_categoria "
				." Where d.id_prehsol =".$idprehsol." "
				/*."And id_cc = ".$idcc." "
