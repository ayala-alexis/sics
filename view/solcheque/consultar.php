...
<h4 class="text-blue">Listado de Solicitudes de Cheques de Centro de Costos</h4>
<div class="row" style="margin-bottom: 5px;">
    ...
    <div class="col-sm-2">
        ...
        <select class="form-control input-sm" id="estado" name="estado" style="min-width: 100%;">
            ...
            <option value="start">En Proceso</option>
        </select>
    </div>
    <div class="col-sm-2">
        <br/>
        <a href="?c=solcheque&a=consultar" class="btn btn-success btn-update">
	...
	<tbody>
		...
		<tr style="font-size: 10px;">
			...
			<td>
                            ...
                            <br/>
                            <span class="text-cc">( <?php echo $solc->empresa->cc->codigo;?> )</span>
			...
			<td><?php echo $solc->fecha." ".$solc->hora; ?></td>
                        ...
                        ?>
                        <td style="max-width: 100px;font-size: 10px;color: <?php echo $color;?>">
                            <?php echo status_descripcion($solc->avance,$solc->status);?>
                        </td>
			<td>
                            <a href="?c=solcheque&a=detalle&s=<?php echo $solc->id?>&return=consultar" class="btn btn-sm btn-default" title="Visualizar solicitud">
    ...
    <nav aria-label="Page navigation" class="pull-right">
        <ul class="pagination">
            <li <?php echo ($paginador['pagina_actual']==0 ? "class='disabled'" : "")?>>
                <a href="?c=solcheque&a=consultar&pag=<?php echo ($paginador['pagina_actual']-1)?>&e=<?php echo $id_empresa?>&cc=<?php echo $id_cc?>&s=<?php echo $id_estado?>&anio=<?php echo $id_anio?>&mes=<?php echo $id_mes?>" aria-label="Previous" title="Anterior">
            ...
            <?php for($pag=0;$pag<$paginador['pagina_total'];$pag++):?>
                ...
                <li <?php echo ($pag==$paginador['pagina_actual'] ? "class='active'" : "")?>>
                    <a href="?c=solcheque&a=consultar&pag=<?php echo $pag?>&e=<?php echo $id_empresa?>&cc=<?php echo $id_cc?>&s=<?php echo $id_estado?>&anio=<?php echo $id_anio?>&mes=<?php echo $id_mes?>"><?php echo ($pag+1)?></a>
                </li>
                <?php $totalpag--;?>
                <?php endif;?>
            <?php endfor;?>
            <li <?php echo ($paginador['pagina_actual']>=($paginador['pagina_total']-1) ? "class='disabled'" : "")?>>
                <a href="?c=solcheque&a=consultar&pag=<?php echo ($paginador['pagina_actual']+1)?>&e=<?php echo $id_empresa?>&cc=<?php echo $id_cc?>&s=<?php echo $id_estado?>&anio=<?php echo $id_anio?>&mes=<?php echo $id_mes?>" aria-label="Next" title="Siguiente">
...
<script>
    $(document).ready(function(){
        ...
        $('.btn-update').click(function(){
            $(this).attr('href','?c=solcheque&a=consultar&e=' + inp_empresa.val() + '&cc=' + inp_cc.val() + '&s=' + inp_estado.val() + '&anio=' + inp_anio.val() + '&mes=' + inp_mes.val());
        });
        var pjson={
            empresa_cc: function(){
                    if(inp_empresa.val()!==''){
